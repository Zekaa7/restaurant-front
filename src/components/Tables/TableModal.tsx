import React from "react";
import type { Table, TableStatus } from "./TableManagment";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
import TableIcon from "../../assets/icons/table_restaurant.svg";
import { getStatusLabel, getStatusColor, ipHome } from "../../helper";
import { authFetch } from "../../FetchApis";

interface TableModalProps {
  table: Table | null;
  onClose: () => void;
  onStatusChange: (tableId: number, status: TableStatus) => void;
}

const TableModal = ({ table, onClose, onStatusChange }: TableModalProps) => {
  const [reservationDateTime, setReservationDateTime] =
    React.useState<Date | null>(null);
  if (!table) return null;

  //Post req za promenu iz slobodnog u rezervisano
  const handleConfirmReservation = async () => {
    const res = await authFetch(`${ipHome}/tables/${table.table_id}/reserve`, {
      method: "PATCH",
    });
    if (!res.ok) {
      throw new Error("Rezervacija nije uspela");
    }
    const data = await res.json();
    onStatusChange?.(table.table_id, "RESERVED");

    onClose();
  };
  const handleSetNotInUse = async () => {
    try {
      const res = await authFetch(
        `${ipHome}/tables/${table.table_id}/disable`,
        { method: "PATCH" }
      );

      if (!res.ok) {
        throw new Error("Promena statusa nije uspela");
      }

      onStatusChange(table.table_id, "NOTINUSE");
      onClose();
    } catch (err) {
      console.error(err);
    }
  };

  const handleSetFree = async () => {
    try {
      const res = await authFetch(`${ipHome}/tables/${table.table_id}/`, {
        method: "PATCH",
      });
      if (!res.ok) {
        throw new Error("Promena statusa nije uspela");
      }
      onStatusChange(table.table_id, "FREE");
    } catch (error) {
      console.error(error);
    }
  };

  return (
    <div
      className="fixed inset-0 bg-black/60 flex items-center justify-center z-50"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 max-w-xl w-full mx-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center mb-5">
          <h3 className="text-2xl font-bold flex items-center text-gray-800">
            Sto broj {table.table_id}{" "}
            {
              <img
                src={TableIcon}
                alt="Stolovi"
                className="w-8 h-8 mx-auto"
                style={{
                  filter:
                    "invert(26%) sepia(51%) saturate(2872%) hue-rotate(347deg) brightness(94%) contrast(90%)", // ≈ #8B4513 saddlebrown
                }}
              />
            }
          </h3>
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-gray-800 text-2xl leading-none"
          >
            ×
          </button>
        </div>

        <div className="space-y-5">
          <div>
            <span className="text-gray-600 ">Status:</span>{" "}
            <span className="font-semibold">
              {getStatusLabel(table.status)}
            </span>
          </div>

          {table.status === "FREE" && (
            <div className="space-y-6">
              {/* Rezervacija */}
              <div className="rounded-xl bg-gray-50 p-5 shadow-sm border border-gray-200 text-center">
                <label className="block text-sm font-semibold text-gray-700 mb-3">
                  Datum i vreme rezervacije
                </label>

                <DatePicker
                  selected={reservationDateTime}
                  onChange={(date: Date | null) => setReservationDateTime(date)}
                  showTimeSelect
                  timeFormat="HH:mm"
                  timeIntervals={30}
                  dateFormat="dd.MM.yyyy HH:mm"
                  minDate={new Date()}
                  placeholderText="Izaberi datum i vreme"
                  className="w-full rounded-lg border px-4 py-2"
                />
              </div>

              <button
                onClick={handleConfirmReservation}
                disabled={!reservationDateTime}
                className={`w-full py-3 text-white font-medium rounded-lg ${
                  reservationDateTime
                    ? "bg-emerald-600 hover:bg-emerald-700"
                    : "bg-gray-300 cursor-not-allowed"
                }`}
              >
                Rezerviši sto
              </button>

              {/* Separator */}
              <div className="flex items-center gap-3">
                <div className="flex-1 h-px bg-gray-300" />
                <span className="text-xs text-gray-500">ILI</span>
                <div className="flex-1 h-px bg-gray-300" />
              </div>

              {/* NOT IN USE */}
              <button
                onClick={handleSetNotInUse}
                className="w-full py-3 bg-gray-500 hover:bg-gray-600 text-white font-medium rounded-lg transition"
              >
                Stavi van upotrebe
              </button>
            </div>
          )}

          {table.status === "RESERVED" && (
            <button
              onClick={() => {
                // ovdje možeš imati logiku za potvrdu dolaska
                onClose();
              }}
              className={`w-full py-3 text-white font-medium rounded-lg transition ${getStatusColor(
                "RESERVED"
              )}`}
            >
              Potvrdi dolazak
            </button>
          )}

          {table.status === "NOTINUSE" && (
            <button
              onClick={handleSetFree}
              className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-medium rounded-lg transition"
            >
              Ponovo aktiviraj sto
            </button>
          )}

          {/* 
          {table.status === "OCCUPIED" && (
            <button
              onClick={() => {
                // onCheckout?.(table.id);
                onClose();
              }}
              className={`w-full py-3 text-white font-medium rounded-lg transition ${getStatusColor(
                "OCCUPIED"
              )}`}
            >
              Zatvori račun
            </button>
          )} */}
          <button
            onClick={onClose}
            className="w-full py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium rounded-lg transition"
          >
            Zatvori
          </button>
        </div>
      </div>
    </div>
  );
};

export default TableModal;
